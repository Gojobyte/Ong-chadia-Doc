...
- [ ] **Task 1: Créer les schémas Prisma** (AC: 1, 2)
  - [ ] Ajouter modèle `ShareLink` dans `prisma/schema.prisma` (avec maxAccessCount et revokedAt)
  - [ ] Ajouter modèle `AccessLog` dans `prisma/schema.prisma`
  - [ ] Créer et exécuter la migration

- [ ] **Task 2: Créer les types partagés** (AC: 1-4, 6, 7)
  ...
  - [ ] Définir enum `MaxAccessCount` (1, 5, 10, null=illimité)
  - [ ] Créer `packages/shared/src/types/access-log.types.ts`
  - [ ] Définir `AccessLogResponse`, enum `AccessAction`

- [ ] **Task 3: Créer le service sharing** (AC: 3, 4, 7, 8, 11)
  - [ ] Créer `apps/api/src/modules/documents/sharing.service.ts`
  - [ ] Implémenter `createShareLink(documentId, expiresIn, maxAccessCount, userId)`
  - [ ] Générer token sécurisé avec `crypto.randomBytes`
  - [ ] Implémenter `getDocumentByShareToken(token)` - vérifie expiration, révocation et max accès
  - [ ] Implémenter `incrementAccessCount(shareLink)` - vérifie si max atteint
  - [ ] Implémenter `revokeShareLink(linkId, userId)` - met à jour revokedAt
  - [ ] Implémenter `getShareLinks(documentId)` - liste les liens actifs
...
- [ ] **Task 6: Créer les routes sharing** (AC: 3, 4, 8, 11)
  - [ ] `POST /api/documents/:id/share` → `createShareLink` (Staff/Super-Admin)
  - [ ] `GET /api/share/:token` → `downloadByShareToken` (public, pas d'auth)
  - [ ] `GET /api/documents/:id/share-links` → `getShareLinks` (Staff/Super-Admin)
  - [ ] `DELETE /api/documents/:id/share/:linkId` → `revokeShareLink` (Staff/Super-Admin)
  - [ ] Vérifier permission ADMIN ou WRITE sur le document pour créer/révoquer un lien

- [ ] **Task 7: Créer la route access-logs** (AC: 8)
  - [ ] `GET /api/documents/:id/access-logs` → `getAccessLogs` (Super-Admin only)
  - [ ] Pagination supportée

- [ ] **Task 8: Créer le composant ShareButton UI** (AC: 5, 6, 7)
  - [ ] Créer `apps/web/src/components/documents/ShareButton.tsx`
  - [ ] Popover avec options de durée d'expiration
  - [ ] Popover avec options de nombre d'accès max (1, 5, 10, illimité)
  - [ ] Affiche le lien généré avec bouton copier

- [ ] **Task 8b: Créer le composant ShareLinksManager UI** (AC: 11, 13)
  - [ ] Créer `apps/web/src/components/documents/ShareLinksManager.tsx`
  - [ ] Tableau listant les liens actifs : token (tronqué), expire le, accès restants, créé par
  - [ ] Bouton "Révoquer" pour chaque lien avec confirmation
