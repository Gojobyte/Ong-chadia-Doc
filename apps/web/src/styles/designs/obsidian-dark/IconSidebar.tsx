...
function getRoleBadge(role?: Role): { label: string; gradient: string } {
  switch (role) {
    case Role.SUPER_ADMIN:
      return { label: 'Admin', gradient: 'from-violet-400 to-purple-500' };
    case Role.STAFF:
      return { label: 'Staff', gradient: 'from-cyan-400 to-cyan-600' };
    default:
...
export function IconSidebar() {
  ...
  const mainNavItems = [
    { icon: LayoutDashboard, label: 'Dashboard', href: '/dashboard', gradient: 'from-cyan-400 to-cyan-600' },
    { icon: FileText, label: 'Documents', href: '/documents', gradient: 'from-violet-400 to-purple-500' },
    { icon: Star, label: 'Favoris', href: '/favorites', gradient: 'from-amber-400 to-orange-500' },
    { icon: FolderKanban, label: 'Projets', href: '/projects', gradient: 'from-emerald-400 to-green-500' },
  ];
  ...
  return (
    <aside className="w-64 bg-[#0f0f14] flex flex-col flex-shrink-0 border-r border-white/[0.06]">
      ...
      <div className="px-5 py-6">
        <div className="flex items-center gap-3">
          <div className="relative group">
            <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-cyan-400 to-cyan-600 flex items-center justify-center shadow-lg shadow-cyan-500/30 transition-transform duration-300 group-hover:scale-105">
              <img
                src="/logo1.png"
                alt="CDT"
            ...
            {/* Status indicator */}
            <div className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-emerald-400 rounded-full border-[3px] border-[#0f0f14] shadow-lg shadow-emerald-500/40" />
          </div>
      ...
      <div className="px-4 mb-5">
        <button className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-cyan-400 to-cyan-600 hover:from-cyan-300 hover:to-cyan-500 text-white text-sm font-semibold shadow-lg shadow-cyan-500/30 transition-all duration-300 hover:shadow-cyan-500/50 hover:scale-[1.02] active:scale-[0.98] group glow-cyan">
        ...
        <div className="px-3 py-3 border-t border-white/[0.06]">
          <div className="flex items-center gap-3 px-2.5 py-2.5 rounded-xl hover:bg-white/[0.04] transition-all duration-200 cursor-pointer group">
            ...
            <div className="relative">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-cyan-400 to-cyan-600 flex items-center justify-center text-sm font-bold text-white shadow-lg shadow-cyan-500/25">
                {getInitials(user?.firstName, user?.lastName)}
              </div>
              <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-emerald-400 rounded-full border-2 border-[#0f0f14]" />
            </div>
